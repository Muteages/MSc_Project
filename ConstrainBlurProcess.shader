Shader "Hidden/ConstrainBlurProcess"
{
	Properties
	{
...
#include "Utils.cginc"
	...
	float4 fragment(in Varyings input) : SV_Target
	{
		...
		//		return float4(0, 0, 0, 1);
			...
			Y = ((lod > (_darkScale - _transitionSize)) && (lod < _darkScale)) ? 
				(1 - (lod - (_darkScale - _transitionSize)) / _transitionSize) * Y + (lod - (_darkScale - _transitionSize)) / _transitionSize * 0.7 
				: Y;

			Y = lod >= _darkScale ? 0.7 : Y;
